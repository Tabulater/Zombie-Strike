...
TIME_FONT_SIZE = 24
PLAYER_SPEED = 3
PLAYER_RUN_SPEED = 15  # Increased from 9 to 15 for faster sprinting
...
class SoundManager:
    ...
    Handles loading, playing, and volume control of all game sounds.
    """
    def __init__(self):
        ...
        self.load_sounds()  # Load all game sounds
        
    def load_sounds(self):
        """
        Loads all game sound effects and music files.
        Sets initial volume levels for each sound.
        """
        # Load sound effects
        ...
        for sound_name, sound in self.sounds.items():
            if sound_name == 'grenade':
                sound.set_volume(5.0)  # Set grenade sound to extremely high volume
            else:
    ...
    def play_sound(self, sound_name: str):
        ...
        For grenade sounds, plays multiple times for extra impact.
        ...
        if sound_name in self.sounds:
            if sound_name == 'grenade':
                # Stop any currently playing grenade sounds
                self.sounds[sound_name].stop()
                # Play the grenade sound multiple times for extra impact
                self.sounds[sound_name].play()
                pygame.time.delay(50)  # Small delay to ensure sound plays
                self.sounds[sound_name].play()
            else:
                self.sounds[sound_name].play()
    ...
    def play_music(self, music_name: str, loops: int = -1):
        """
        Plays a music track with specified number of loops.
...
class HomePage:
    def __init__(self, screen, clock):
        ...
        self.options = ["Start Game", "Settings", "About", "Quit"]
        ...
        try:
            self.military_font = pygame.font.Font("assets/BAUHS93.TTF", 72)
        except:
    ...
    def update_particles(self):
        for particle in self.particle_system:
            ...
            if particle['pos'][0] < 0:
                particle['pos'] = (self.screen_width, particle['pos'][1])
            elif particle['pos'][0] > self.screen_width:
                particle['pos'] = (0, particle['pos'][1])
            if particle['pos'][1] < 0:
                particle['pos'] = (particle['pos'][0], self.screen_height)
            elif particle['pos'][1] > self.screen_height:
                particle['pos'] = (particle['pos'][0], 0)
    def draw_particles(self):
        for particle in self.particle_system:
            ...
            self.screen.blit(surface, particle['pos'])
    ...
    def draw_box_with_effects(self, width, height, x, y, border_color=(255, 255, 255)):
        ...
        inner_border = 10
        pygame.draw.rect(box_surface, (200, 200, 200, 150), 
                        (inner_border, inner_border, width-2*inner_border, height-2*inner_border), 2)
    ...
    def draw_instructions_button(self):
        ...
        for i in range(2):
            alpha = 150 - i * 50
            pygame.draw.rect(button_surface, (255, 255, 255, alpha), 
                           (i, i, button_width-i*2, button_height-i*2), 2)
        question_mark = "?"
        font_size = 32
    ...
    def draw_volume_controls(self):
        # Draw volume button
        button_width = button_height = 40
        button_x = self.screen_width - button_width - 70  # Position next to instructions button
        button_y = 20
        button_surface = self.create_gradient_surface(button_width, button_height, 200, 100)
        for i in range(2):
            alpha = 150 - i * 50
        ...
        # Draw speaker icon
        speaker_color = (255, 215, 0) if self.volume_button_hovered else (200, 200, 200)
        pygame.draw.polygon(button_surface, speaker_color, [
        ...
        self.screen.blit(button_surface, (button_x, button_y))
        self.volume_button_rect = pygame.Rect(button_x, button_y, button_width, button_height)

        # Draw volume slider if button is hovered
        if self.volume_button_hovered:
            slider_width = 150
            slider_height = 20
            slider_x = button_x - slider_width - 10
            slider_y = button_y + (button_height - slider_height) // 2
            
            # Draw slider background
            slider_bg = pygame.Surface((slider_width, slider_height), pygame.SRCALPHA)
            slider_bg.fill((0, 0, 0, 150))
            pygame.draw.rect(slider_bg, (255, 255, 255, 100), (0, 0, slider_width, slider_height), 2)
            
            # Draw volume level
            volume_width = int(slider_width * self.sound_manager.volume)
            for x in range(volume_width):
                progress = x / slider_width
                color = (
                    int(255 * (1 - progress)),
                    int(255 * progress),
                    0,
                ...
                pygame.draw.line(slider_bg, color,
                               (x, 0),
                               (x, slider_height))
            
            # Draw slider handle
            handle_x = int(slider_width * self.sound_manager.volume)
            pygame.draw.circle(slider_bg, (255, 255, 255),
                             (handle_x, slider_height // 2), 8)
            
            self.screen.blit(slider_bg, (slider_x, slider_y))
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        if selected == "Quit":
                            ...
                            if not self.settings_screen:
                                self.settings_screen = SettingsScreen(self.screen, self.clock, self.sound_manager)
                            action = self.settings_screen.run()
                            if action == "menu":
                                self.settings_screen = None
                            return action
                        elif selected == "About":
    ...
    def draw_menu(self):
        ...
        self.draw_text("OPERATION: ZOMBIE STRIKE", 
                      self.screen_width//2, 
                      self.screen_height//4, 
                      (255, 215, 0), 
                      self.military_font)
        self.draw_text("TACTICAL SURVIVAL", 
                      self.screen_width//2, 
                      self.screen_height//4 + 60, 
                      (107, 142, 35), 
                      self.military_font)
        ...
        for i, control in enumerate(controls):
            self.draw_text(control, 
                          self.screen_width//2, 
                          self.screen_height*3//4 + i*30, 
                          (200, 200, 200),
                          self.small_font)
        self.draw_text("v1.0.0", 
    ...
    def draw_instructions(self):
        ...
        self.screen.blit(content_surface, (box_x + 20, box_y + 20), 
                        (0, self.instructions_scroll, box_width - 40, box_height - 40))
        if content_height > box_height - 40:
            ...
            pygame.draw.rect(self.screen, (50, 50, 50), 
                           (box_x + box_width - 30, box_y + 20, 
                            scroll_bar_width, box_height - 40))
            for y in range(int(scroll_bar_height)):
                alpha = int(200 * (1 - y/scroll_bar_height))
                pygame.draw.line(self.screen, (200, 200, 200, alpha),
                               (box_x + box_width - 30, scroll_bar_y + y),
                               (box_x + box_width - 20, scroll_bar_y + y))
...
class Player:
    ...
    def take_damage(self, amount: int) -> None:
        ...
        if current_time - self.last_damage_time >= self.damage_cooldown:
            ...
            self.create_hit_effect(SCREEN_WIDTH // 2, self.rect.centery)

    def create_hit_effect(self, x: int, y: int) -> None:
        """
        Creates a visual effect when player takes damage.
...
class Game:
        ...
        for zombie in self.zombies[:]:
            ...
            for bullet in self.player.bullets[:]:
                ...
                if zombie.is_alive and bullet_world_rect.colliderect(zombie.rect):
                    ...
                    self.player.create_hit_effect(bullet_world_rect.centerx - self.world_offset, bullet_world_rect.centery)
                    self.bullets_hit += 1
                    break
            for explosion in self.player.explosions:
                ...
                if zombie.is_alive and explosion_rect.colliderect(zombie.rect):
                    ...
                    self.player.create_hit_effect(zombie.rect.centerx - self.world_offset, zombie.rect.centery)
    ...
    def draw_projectiles(self) -> None:
        ...
        for effect in self.player.hit_effects:
            pygame.draw.circle(screen, WHITE, effect['pos'], effect['radius'])
            pygame.draw.circle(screen, MILITARY_RED, effect['pos'], effect['radius'] // 2)
...
class SettingsScreen:
    ...
    def draw_slider(self, x, y, width, height, value, dragging, label):
        # Draw label
        self.draw_text(label, x - 10, y - 25, WHITE, self.small_font, False)
        
        # Draw slider background
        slider_bg = pygame.Surface((width, height), pygame.SRCALPHA)
        slider_bg.fill((0, 0, 0, 150))
        pygame.draw.rect(slider_bg, (255, 255, 255, 100), (0, 0, width, height), 2)
        
        # Draw volume level with gradient
        volume_width = int(width * value)
        ...
        # Draw slider handle
        handle_x = int(width * value)
        handle_color = (255, 215, 0) if dragging else (255, 255, 255)
        pygame.draw.circle(slider_bg, handle_color,
                         (handle_x, height // 2), 8)
    ...
    def draw_menu(self):
        ...
        # Draw title
        self.draw_text("SETTINGS", self.screen_width//2, 50, HIGHLIGHT, self.military_font)
        
        # Draw volume controls
        slider_width = 300
        slider_height = 20
        start_x = self.screen_width//2 - slider_width//2
        start_y = 150
        
        # Master Volume
        self.volume_slider_rect = self.draw_slider(
            start_x, start_y,
            slider_width, slider_height,
            self.sound_manager.volume,
            self.volume_slider_dragging,
            "Master Volume"
        )
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.volume_slider_rect:
                    if self.volume_slider_rect.collidepoint(event.pos):
                        ...
                        volume = max(0.0, min(1.0, relative_x / self.volume_slider_rect.width))
...
class AboutScreen:
    ...
    def draw_menu(self):
        ...
        # Draw title
        self.draw_text("ABOUT", self.screen_width//2, 50, HIGHLIGHT, self.military_font)
...
def main():
    ...
    while True:
        ...
        if action == "quit":
            ...
            homepage.sound_manager.stop_music()
